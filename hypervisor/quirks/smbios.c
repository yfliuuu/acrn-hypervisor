...
static int copy_smbios_to_guest(struct acrn_vm *vm)
{
    ...
    if (smbios_eps_size) {
         ...
         */
        uint64_t klstart = (uint64_t)vm->sw.kernel_info.kernel_load_addr;
        uint64_t klend = (uint64_t)vm->sw.kernel_info.kernel_load_addr + vm->sw.kernel_info.kernel_size + MEM_2M;
        uint64_t mlstart = max((uint64_t)vm->sw.ramdisk_info.load_addr, MEM_1M);
        uint64_t mlend = max((uint64_t)vm->sw.ramdisk_info.load_addr + vm->sw.ramdisk_info.size, MEM_1M);
        uint64_t low = min(klstart, mlstart);
        uint64_t high = max(klend, mlend);

        gpa = find_space_from_ve820(vm, smbios_table_size, MEM_1M, low);
        if (gpa == INVALID_GPA) {
            gpa = find_space_from_ve820(vm, smbios_table_size, high, MEM_4G);
            if (gpa == INVALID_GPA) {
                low = max(min(klstart, mlend), min(klend, mlstart));
                high = min(max(klstart, mlend), max(klend, mlstart));
                gpa = find_space_from_ve820(vm, smbios_table_size, low, high);
                if (gpa == INVALID_GPA) {
                    pr_err("Not enough memory to place SMBIOS information to guest %d", vm->vm_id);
                }
            }
        }
